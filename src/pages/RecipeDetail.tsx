        ...
        <div className="mx-auto w-full max-w-5xl snap-start scroll-mt-16 px-4 sm:px-6">
          <section className="py-8">
            <SectionHead
              ...
              subtitle="Pick a spice level, or follow one of our best cooks — just one, not both."
            />
            <TasteAndCreators
              variants={recipe.variants}
              selectedVariantId={selection.variantId}
              onSelectVariant={(id) => {
                // A spice-level chip and a chef's pick are two ways to answer the
                // same question, not two independent toggles — choosing one clears
                // the other so only one thing ever reads as "selected".
                selection.setVariant(id)
                selection.setCreator(null)
              }}
